...
func (a *AnimeQuery) collectField(ctx context.Context, opCtx *graphql.OperationContext, collected graphql.CollectedField, path []string, satisfies ...string) error {
	...
	for _, field := range graphql.CollectFields(opCtx, collected.Selections, satisfies) {
		...
		case "episodes":
			...
			pager, err := newEpisodePager(args.opts, args.last != nil)
			if err != nil {
			...
			if hasCollectedField(ctx, append(path, totalCountField)...) || hasCollectedField(ctx, append(path, pageInfoField)...) {
				...
				if hasPagination || ignoredEdges {
					query := query.Clone()
					a.loadTotal = append(a.loadTotal, func(ctx context.Context, nodes []*Anime) error {
						ids := make([]driver.Value, len(nodes))
						for i := range nodes {
							ids[i] = nodes[i].ID
						}
						var v []struct {
							NodeID int `sql:"anime_id"`
							Count  int `sql:"count"`
						}
						query.Where(func(s *sql.Selector) {
							s.Where(sql.InValues(s.C(anime.EpisodesColumn), ids...))
						})
						if err := query.GroupBy(anime.EpisodesColumn).Aggregate(Count()).Scan(ctx, &v); err != nil {
							return err
						}
					...
					a.loadTotal = append(a.loadTotal, func(_ context.Context, nodes []*Anime) error {
						for i := range nodes {
			...
			if limit := paginateLimit(args.first, args.last); limit > 0 {
				modify := limitRows(anime.EpisodesColumn, limit, pager.orderExpr(query))
...
func newAnimePaginateArgs(rv map[string]any) *animePaginateArgs {
	...
	if v, ok := rv[orderByField]; ok {
		...
		case map[string]any:
			var (
				err1, err2 error
				order      = &AnimeOrder{Field: &AnimeOrderField{}, Direction: entgql.OrderDirectionAsc}
			)
			...
			if err1 == nil && err2 == nil {
				args.opts = append(args.opts, WithAnimeOrder(order))
			}
		case *AnimeOrder:
			if v != nil {
	...
	for _, field := range graphql.CollectFields(opCtx, collected.Selections, satisfies) {
		...
		case "episodes":
			...
			if hasCollectedField(ctx, append(path, totalCountField)...) || hasCollectedField(ctx, append(path, pageInfoField)...) {
				...
				if hasPagination || ignoredEdges {
					query := query.Clone()
					rg.loadTotal = append(rg.loadTotal, func(ctx context.Context, nodes []*ReleaseGroup) error {
						ids := make([]driver.Value, len(nodes))
						for i := range nodes {
							ids[i] = nodes[i].ID
						}
						var v []struct {
							NodeID int `sql:"release_group_id"`
							Count  int `sql:"count"`
						}
						query.Where(func(s *sql.Selector) {
							s.Where(sql.InValues(s.C(releasegroup.EpisodesColumn), ids...))
						})
						if err := query.GroupBy(releasegroup.EpisodesColumn).Aggregate(Count()).Scan(ctx, &v); err != nil {
							return err
						}
						m := make(map[int]int, len(v))
						for i := range v {
					...
					rg.loadTotal = append(rg.loadTotal, func(_ context.Context, nodes []*ReleaseGroup) error {
						for i := range nodes {
							n := len(nodes[i].Edges.Episodes)
							if nodes[i].Edges.totalCount[0] == nil {
								nodes[i].Edges.totalCount[0] = make(map[string]int)
			...
			if limit := paginateLimit(args.first, args.last); limit > 0 {
				modify := limitRows(releasegroup.EpisodesColumn, limit, pager.orderExpr(query))
				query.modifiers = append(query.modifiers, modify)
			} else {
				query = pager.applyOrder(query)
			}
...
func newReleaseGroupPaginateArgs(rv map[string]any) *releasegroupPaginateArgs {
	...
	if v, ok := rv[orderByField]; ok {
		...
		case map[string]any:
			var (
				err1, err2 error
				order      = &ReleaseGroupOrder{Field: &ReleaseGroupOrderField{}, Direction: entgql.OrderDirectionAsc}
			)
			if d, ok := v[directionField]; ok {
				err1 = order.Direction.UnmarshalGQL(d)
			}
			...
			if err1 == nil && err2 == nil {
				args.opts = append(args.opts, WithReleaseGroupOrder(order))
			}
		case *ReleaseGroupOrder:
			if v != nil {
