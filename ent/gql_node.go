...
func (c *Client) noder(ctx context.Context, table string, id int) (Noder, error) {
	...
	case anime.Table:
		query := c.Anime.Query().
			Where(anime.ID(id))
		query, err := query.CollectFields(ctx, animeImplementors...)
		if err != nil {
			return nil, err
		}
		...
		return n, nil
	case episode.Table:
		query := c.Episode.Query().
			Where(episode.ID(id))
		query, err := query.CollectFields(ctx, episodeImplementors...)
		if err != nil {
			return nil, err
		}
		n, err := query.Only(ctx)
	...
	case item.Table:
		...
		return n, nil
	case releasegroup.Table:
		query := c.ReleaseGroup.Query().
			Where(releasegroup.ID(id))
		query, err := query.CollectFields(ctx, releasegroupImplementors...)
		if err != nil {
			return nil, err
		}
		n, err := query.Only(ctx)
