...
func (aq *AnimeQuery) QueryEpisodes() *EpisodeQuery {
...
func (aq *AnimeQuery) Clone() *AnimeQuery {
	...
	return &AnimeQuery{
		...
		withEpisodes: aq.withEpisodes.Clone(),
...
func (aq *AnimeQuery) WithEpisodes(opts ...func(*EpisodeQuery)) *AnimeQuery {
	query := (&EpisodeClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	aq.withEpisodes = query
	return aq
}
...
func (aq *AnimeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Anime, error) {
	...
	if query := aq.withEpisodes; query != nil {
...
func (aq *AnimeQuery) loadEpisodes(ctx context.Context, query *EpisodeQuery, nodes []*Anime, init func(*Anime), assign func(*Anime, *Episode)) error {
	...
	query.Where(predicate.Episode(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(anime.EpisodesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.anime_id
		if fk == nil {
			return fmt.Errorf(`foreign-key "anime_id" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
